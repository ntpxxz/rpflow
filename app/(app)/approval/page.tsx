...
export default function ApprovalPage() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      // Find the request ID associated with this step to redirect
      const step = allSteps.find(s => s.id === currentAction.stepId);
      if (step) {
        router.push(`/purchase-requests/${step.requestId}`);
      } else {
        // Fallback if step not found (unlikely)
        fetchRequests();
      }
                    ...
                    <TableBody>
                      {paginatedDoneSteps.length === 0 ? (
                        ...
                        paginatedDoneSteps.map((step) => (
                          ...
                          >
                            {/* ... Similar columns ... */}
                            <TableCell className="py-4 pl-6 font-medium text-foreground text-sm"><div className="flex items-center gap-2">
                              <div className="p-1.5 rounded bg-slate-100 text-slate-500 group-hover:bg-white group-hover:text-orange-600 transition-colors">
                                <FileText className="h-4 w-4" />
                              </div>
                              {step.request.id}
